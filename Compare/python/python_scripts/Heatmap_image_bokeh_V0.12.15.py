...
'''Heatmaps for on MIDAS where bokeh is not latest version yet, eg bokeh version 0.12.15'''
...
def plot_heatmap_image(NOG_matrix, row_clusters, data_main, parameters):
    ...
    h.select_one(HoverTool).tooltips = [('Phylum', '@phylum_MMETSP')]
    ...
    p = figure(x_range=(np.array(NOG_matrix.columns)),
               ...
               y_range=h.y_range,
               tools=tools_to_show, x_axis_location="above")
