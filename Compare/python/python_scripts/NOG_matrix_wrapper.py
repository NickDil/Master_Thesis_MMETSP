...
def get_param():
    ...
    parser.add_argument('-min_completeness', '-mc', default=0, type=str, help='minimun completeness score for experiment to be included. Default 0; include all')
...
def NOG_vector_R(args):
    ...
    command = 'Rscript'
    path2script = args['project_script_location'] + 'Compare/R/R_scripts/NOG_vector_generator.R'
    ...
    min_completeness = str(args['min_completeness'])
    ...
    L = [infile, dir_base, GO_term ,min_completeness, min_occurence, dir_out, COUNT, data_normalize]
...
def main():
    ...
    print("\n[START] Creating NOG feature matrix:\t", strftime("%Y-%m-%d %H:%M:%S", gmtime()), "\n")
    ...
    print("[STOP] Creating NOG feature matrix:\t", strftime("%Y-%m-%d %H:%M:%S", gmtime()), "\n")
