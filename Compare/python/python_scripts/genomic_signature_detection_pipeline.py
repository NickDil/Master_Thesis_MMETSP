...
def main(parameters):
    ...
    parameters3 = {'input_table': 'NOG_MATRIX_minocc4_None_counts.txt', 'flame_path': '../../../../flame-clustering/sample', 'axis': "10", 'd': '1', 'knn': '4', 'write': True, 'outdir': None, 'keep': False}
    for param in parameters:
        if param in parameters3:
            parameters3[param] = parameters[param]
            if parameters[param].upper() == "TRUE":
                parameters3[param] = True
            elif parameters[param].upper() == "FALSE":
                parameters3[param] = False

    # Change output dir
    parameters3['outdir'] = parameters2["outdir"]+NOG_MATRIX+"/"

    FLAME_clustering.main(parameters3)
    ...
    cluster_list = "{dir}FLAME_output_d{d}_knn{knn}_axis{ax}/FLAME_output_d{d}_knn{knn}_axis0.txt".format(dir=parameters3['outdir'], d=parameters3['d'], knn=parameters3['knn'], ax =parameters3['axis'])

    command = [parameters["enricher_location"], ftr_file, cluster_list] + parameters["arguments_string"].split(" ")
    x = subprocess.check_output(command, universal_newlines=True)

    enricher_filepath = "{dir}/enricher_output_minocc{mo}_{go}{cs}_d{d}knn{knn}ax{ax}.txt".format(dir="/".join(cluster_list.split("/")[:-1]),
                                                                    ...
                                                                    knn=parameters3['knn'],
                                                                    ax=parameters3['axis'])
    f = open(enricher_filepath, 'w+')
    f.write(x)
    f.close()
    print("[INFO] Output enrichment analysis written to:", enricher_filepath)
    ...
    parameters4 = {'input_table': None, 'outdir': None, 'row_order': cluster_list, 'col_order': None,
    ...
    # Change output dir
    parameters4['outdir'] = parameters3["outdir"]
