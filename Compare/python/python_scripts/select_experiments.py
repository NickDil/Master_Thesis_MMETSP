...
def get_param():
    ...
    parser.add_argument('-outdir', '-o', type=str, help='Defaults to working directory. Output directory for the results, figures if using '
...
def get_R_data(args):
    ...
    if not data_dir_ex:
        ...
        print(strftime("%Y-%m-%d %H:%M:%S", gmtime()), "\t[START]: Selecting experiments with Rscript 'Compare/R/R_scripts/collect_data.R'", "\n")
        ...
        # x = subprocess.check_output(cmd, universal_newlines=True)
        print( strftime("%Y-%m-%d %H:%M:%S", gmtime()), "\t[STOP]: Experiments selected", "\n")

        results_dir = args["outdir"] + "{name}/".format(name=args["name"])

        print("Rscript output:\n", x)

        print("[INFO] Reading generated files:\n")
        print("{name}/Data/{name}_main.csv\n".format(name=args["name"]))
        data_main = pd.read_table(args["outdir"] + "{name}/Data/{name}_main.csv".format(name=args["name"]), sep=",", encoding='latin-1')
        print("{name}/Data/{name}_txcmp.csv\n".format(name=args["name"]))
        data_txcmp = pd.read_table(args["outdir"] + "{name}/Data/{name}_txcmp.csv".format(name=args["name"]), sep=",", encoding='latin-1')
...
def main(args):
    ...
    # Write metadata file for enrichment analysis later
    ftr_file = args["outdir"] + "{name}/Data/metadata.txt".format(name=args["name"])

    data_main[[args["metadata_type"], "title"]].to_csv(ftr_file, sep="\t", encoding='utf-8', index=False, header=False)
    ...
    p = plot_completeness_score(data_main, results_dir)
    save(p)
