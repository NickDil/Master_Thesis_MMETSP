...
arguments_use = c('infile', '/home/nidil/Documents/Project_TRAPID/Scripts/', "None", 1, getwd(), FALSE, FALSE)
names(arguments_use) = c("infile", "dir.base", "GO.term" , "min_occurence", "dir.out", "COUNT", "data.normalize")

# Convert "None" strings to NULL values
infile = arguments[1]
if(!is.na(arguments[2])){arguments_use["dir.base"] = arguments[2]}
if(!is.na(arguments[3])){arguments_use["GO.term"] = arguments[3]}
if(!is.na(arguments[4])){arguments_use["min_occurence"] = arguments[4]}
if(!is.na(arguments[5])){arguments_use["dir.out"] = arguments[5]}
arguments_use["COUNT"] = arguments[6]
if(!is.na(arguments[7])){arguments_use["data.normalize"] = arguments[7]}else{
...
#remove NOGs that are not present or only in `min_occurence` number of samples
...
dir.out1 = paste0("NOG_MATRIX_minocc", arguments_use["min_occurence"],"_", paste(unlist(strsplit(arguments_use["GO.term"], ":")), collapse = "_"))
