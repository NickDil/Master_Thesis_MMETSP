...
python /home/nidil/Documents/trapid_develop/app/scripts/python/run_core_gf_analysis_trapid.py $MY_CLADE $SGE_TASK_ID --label None --species_perc $MY_SPEC --top_hits 5 --naive --output_dir /group/transreg/nidil/experiment_data/$SGE_TASK_ID/completeness/ > "$SGE_TASK_ID/completeness/core_gfs_completeness_$MY_CLADE.$SGE_TASK_ID.$MY_SPEC.out" 2> "$SGE_TASK_ID/completeness/core_gfs_completeness_$MY_CLADE.$SGE_TASK_ID.$MY_SPEC.err"
